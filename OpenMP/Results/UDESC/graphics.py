...
df['SUM_OMP'] = [0.491, 0.285, 0.193, 0.149, 0.124, 0.112, 0.105, 0.102, 0.106]
df['SUM_OMP'] = df['SUM'].apply(lambda parallel: sum_serial / parallel)
...
df['MULT_OMP'] = [72.692, 35.865, 24.450, 18.608, 20.731, 17.477, 16.614, 15.280, 16.432]
df['MULT_OMP'] = df['MULT'].apply(lambda parallel: mult_serial / parallel)
...
df['SORT-bucket_OMP'] = [13.333, 6.985, 4.688, 3.749, 3.700, 3.195, 2.935, 2.668, 2.596]
df['SORT-bucket_OMP'] = df['SORT-bucket'].apply(lambda parallel: sort_serial / parallel)



sns.lineplot(x='N', y='N', data=df)
sns.lineplot(x='N', y='SUM', data=df)
sns.lineplot(x='N', y='SUM_OMP', data=df)
plt.savefig('SUM.png')
plt.clf()

sns.lineplot(x='N', y='N', data=df)
sns.lineplot(x='N', y='MULT', data=df)
sns.lineplot(x='N', y='MULT_OMP', data=df)
plt.savefig('MULT.png')
plt.clf()

sns.lineplot(x='N', y='N', data=df)
sns.lineplot(x='N', y='SORT-bucket', data=df)
sns.lineplot(x='N', y='SORT-bucket_OMP', data=df)
